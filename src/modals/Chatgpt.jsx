...
const ChatGPT = ({ setchatgpt, messages, responses, setMessages, setResponses }) => {
  const chatFunction = async () => {
    try {
      ...
      // Add the new response to chat UI
      setResponses((prev) => [...prev, { text: response.data.response }]);
      setsuccess(true);
    } catch (error) {
      console.error("Local backend failed, using fallback:", error);

      // Fallback to RapidAPI ChatGPT
      try {
        const rapidResponse = await axios.post(
        ...
        console.log("Response from RapidAPI:", rapidResponse.data);

        setResponses((prev) => [
          ...prev,
          { text: rapidResponse.data.response },
        ]);
        setsuccess(true);
      } catch (fallbackError) {
        console.error("Fallback also failed:", fallbackError);
        alert("Sorry, something went wrong");
        setResponses((prev) => [...prev, { text: "Something went wrong" }]);
        setsuccess(true);
      }
    }
  };
